...
class LSVDDriver : public CacheDriver {
public:
  LSVDDriver(Partition& partition_info) : partition_info(partition_info) {}
  virtual ~LSVDDriver() {}
  ...
  virtual int64_t get_free_space(const DoutPrefixProvider* dpp) override { return free_space; }
...
private:
  ...
  rgw::Aio::OpFunc lsvd_cache_read_op(const DoutPrefixProvider *dpp, optional_yield y, rgw::cache::CacheDriver* cache_driver,
				  off_t read_ofs, off_t read_len, const std::string& key);

  rgw::Aio::OpFunc lsvd_cache_write_op(const DoutPrefixProvider *dpp, optional_yield y, rgw::cache::CacheDriver* cache_driver,
